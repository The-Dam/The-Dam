...
function run_byond_tests {
    ...
    run_test_ci "check globals build" "python3 tools/GenerateGlobalVarAccess/gen_globals.py The-Dam.dme code/_helpers/global_access.dm"
    run_test "check globals unchanged" "md5sum -c - <<< '510f3a8215fd2c25c2ddbe95e31bb4c9 *code/_helpers/global_access.dm'"
    run_test "build map unit tests" "scripts/dm.sh -DUNIT_TEST -M$MAP_PATH The-Dam.dme"
    run_test "check no warnings in build" "grep ', 0 warnings' build_log.txt"
    run_test "run unit tests" "DreamDaemon The-Dam.dmb -invisible -trusted -core 2>&1 | tee log.txt"
